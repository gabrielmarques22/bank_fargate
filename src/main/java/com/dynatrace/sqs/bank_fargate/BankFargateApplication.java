...
public class BankFargateApplication {
	...
	public class StringToHexadecimal {
		...
		String getValue(String integer){
			...
			//System.out.println("Converted: "+ Long.toHexString(Integer.parseInt(integer)));
					...
					try {
						...
						for (Message m : messages) {
							...
							try{
								...
								//SpanContext.createFromRemoteParent​(traceid_value, spanid_value, trc_flgs, TraceState.builder().build())
								System.out.println("Span ctx: "+SpanContext.createFromRemoteParent​(traceid_value, spanid_value, trc_flgs, TraceState.builder().build()));
								SpanContext new_span_ctx = SpanContext.createFromRemoteParent​(traceid_value, spanid_value, trc_flgs, TraceState.builder().build());
								
								//Span span_process_msg = tracer.spanBuilder("sqs process msg").startSpan();
								Span span_process_msg = Span.wrap(new_span_ctx);
								Context.current().with(span_process_msg).makeCurrent();
								//.setParent((Context)SpanContext.createFromRemoteParent​(traceid_value, spanid_value, trc_flgs, TraceState.builder().build()))
								//openTelemetry.set
								System.out.println("Span msg:" +span_msg.getSpanContext());
